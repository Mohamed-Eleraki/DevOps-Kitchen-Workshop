...
The FrogTech Security team requests that you implement a scanning stage of the company's main AWS Pipeline, with the criteria below:
...
1. Preparation stage: This includes the installation and preparation steps _(i.e. install AWS CLI, Terraform, Python, Checkov, and Terraform
    _Initialization)_
2. Plan Stage: This includes the Terraform validation and plan commands; The plan must be done using the output file _(i.e. tfplan file)_
3. Scan step action: This includes the Checkov scan and should be run on the plan result, as well as in parallel with the Plan stage.
4. Manual approval: Pause the pipeline until Reviewed & Approved by the checker engineer.
5. Terraform apply: This includes starting to provision resources.
<br><br>
...
2. Append an extra step command to the Plan stage reviewing specific parts of attributes using the jq command.
